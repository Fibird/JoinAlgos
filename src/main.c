...
/** define FK columns in fact table and Dimension vector in dimension table for star join by zys */
//--define rows of SSB tables with SF=100 by zys
...
struct param_t {
    ...
    uint32_t s_seed;
    double skew;
    double updratio;  //--updratio represents update ratio of PK by zys
    double selectivity;  //--selectivity represents selectivity of bitmap on FK table by zys
...
main(int argc, char ** argv)
{
    ...
    cmd_params.skew     = 0.0;
    cmd_params.updratio = 0.0;
    cmd_params.selectivity = 0.0;
...
*/
    ...
    if(cmd_params.fullrange_keys) {
        ...
        else if(cmd_params.selectivity > 0){
            /* S has a bitmap for filtering by zys */
            create_relation_bmfk(&relS, bitm, cmd_params.s_size, 
    ...
    MeasureIndex.column=(vectorkey_t*)malloc(num_lineorder * sizeof(vectorkey_t));
     if (!MeasureIndex.column) { 
      ...
      int i;
      for(i=0;i<num_lineorder;i++) MeasureIndex.column[i]=0;  
      //--assign initiated value of measure index with 1 as vector filter for fact table by zys
      //for(i=0;i<num_lineorder;i++) printf("% d",MeasureIndex.column[i]);  
      //testing initated measure index with all 1 values by zys


/* //--test whether dimension vector are sucessfully created by zys
                     intkey_t * tmpcol;tmpcol=FactColumns[3].column;
                     for(int i=0;i<FactColumns[3].num_tuples;i++) printf("%d ",tmpcol[i]);
                     printf("\nFK column %d tuples!\n",FactColumns[3].num_tuples);
                     vector_t * tmpvec;tmpvec=&MeasureIndex; 
*/                   //  printf("%d ",MeasureIndex->column);
 ...
 //for(int i=0;i<cmd_params.s_size;i++) printf("%d ",bitm[i]);
    ...
    if (cmd_params.updratio!=0) {  //--call AIRU when updratio given a update ratio value by zys
          results = AIRU(&relR, &relS, cmd_params.nthreads,cmd_params.updratio);
    }
    else  if (cmd_params.selectivity!=0) {  
...
print_help(char * progname)
{
    ...
    Other join configuration options, with default values in [] :             \n\
       ...
       --full-range       Spread keys in relns. in full 32-bit integer range  \n\
       --basic-numa       Numa-localize relations to threads (Experimental)   \n\
       --starjoinflag     flag for star join                                  \n\
                                                                              \n\
    Performance profiling options, when compiled with --enable-perfcounters.  \n\
       -p --perfconf=<P>  Intel PCM config file with upto 4 counters [none]   \n\
       -o --perfout=<O>   Output file to print performance counters [stdout]  \n\
...
parse_args(int argc, char ** argv, param_t * cmd_params) 
{
    ...
    while(1) {
        static struct option long_options[] =
            {
                ...
                {"starjoinflag", no_argument,    &starjoinflag, 1},  //--starjoin flag set by zys
        ...
        switch (c)
        {
          ...
          case 'z':
              ...
              break;
          case 'w':
              cmd_params->updratio = atof(optarg);
              break;
          case 'u':
    ...
    cmd_params->starjoinflag   = starjoinflag;
